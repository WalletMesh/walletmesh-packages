...
const responder = await listener.startDiscovery()[0];
...
**Use mocks for the "other" component in announcer/responder pairs:**

```typescript
// For testing DiscoveryResponder, use real announcer with MockEventTarget
...
it('should handle concurrent state transitions', async () => {
  ...
  expect(listener.getCurrentState()).toBe('DISCOVERING');
});
```
...
**API Changes**:
- `DiscoveryInitiator.isDiscoveryInProgress()` now uses 3-state machine
