  ...
  const createAuthWit = useCallback(
    async (
      ...
      try {
        ...
        const authError =
          err instanceof Error ? err : ErrorFactory.unknownError('Failed to create auth witness');
        setError(authError);
        throw authError;
      } finally {
        setIsCreating(false);
  ...
  const createBatchAuthWitnesses = useCallback(
    async (interactions: ContractFunctionInteraction[]): Promise<AuthWitnessWithMetadata[]> => {
      ...
      try {
        ...
        const authError =
          err instanceof Error ? err : ErrorFactory.unknownError('Failed to create batch auth witnesses');
        setError(authError);
        throw authError;
      } finally {
        setIsCreating(false);
  ...
  const createMessageAuthWit = useCallback(
    async (message: string, description?: string): Promise<AuthWitnessWithMetadata> => {
      ...
      try {
        ...
        const authError =
          err instanceof Error ? err : ErrorFactory.unknownError('Failed to create message auth witness');
        setError(authError);
        throw authError;
      } finally {
        setIsCreating(false);
  ...
  const verifyAuthWitness = useCallback(
    async (authWitness: unknown, expectedMessage?: unknown): Promise<boolean> => {
      ...
      try {
        ...
        const verifyError =
          err instanceof Error ? err : ErrorFactory.unknownError('Failed to verify auth witness');
