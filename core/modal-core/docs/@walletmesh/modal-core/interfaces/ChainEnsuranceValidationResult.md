...
[@walletmesh/modal-core](../../../modules.md) / [@walletmesh/modal-core](../README.md) / ChainEnsuranceValidationResult

# Interface: ChainEnsuranceValidationResult
...
> **error**: `null` \| \{ `category`: `"user"` \| `"wallet"` \| `"network"` \| `"general"` \| `"validation"` \| `"sandbox"`; `cause?`: `unknown`; `classification?`: `"network"` \| `"permission"` \| `"provider"` \| `"temporary"` \| `"permanent"` \| `"unknown"`; `code`: `string`; `data?`: `Record`\<`string`, `unknown`\>; `maxRetries?`: `number`; `message`: `string`; `recoveryStrategy?`: `"retry"` \| `"wait_and_retry"` \| `"manual_action"` \| `"none"`; `retryDelay?`: `number`; \}
