...
export class WalletMeshAccount implements NemiAccount {
   ...
   * @internal
   */
  static async create(client: WalletMeshClient, chainId: string): Promise<WalletMeshAccount> {
  ...
  async getCompleteAddress(): Promise<CompleteAddress> {
    if (!this.completeAddressCache) {
      this.completeAddressCache = (await this.provider.call('aztec_getCompleteAddress')) as CompleteAddress;
    }
  ...
  async signMessage(message: Buffer): Promise<Buffer> {
    return (await this.provider.call('aztec_signMessage', [message])) as Buffer;
  }
  ...
  async createAuthWit(messageHash: Fr | Buffer): Promise<AuthWitness> {
    return (await this.provider.call('aztec_createAuthWit', [messageHash])) as AuthWitness;
  }
  ...
  async getChainId(): Promise<Fr> {
    if (!this.chainIdCache) {
      this.chainIdCache = (await this.provider.call('aztec_getChainId')) as Fr;
    }
  ...
  async getVersion(): Promise<Fr> {
    if (!this.versionCache) {
      this.versionCache = (await this.provider.call('aztec_getVersion')) as Fr;
    }
