...
describe('DiscoveryAdapter - New Improvements', () => {
  ...
  describe('Chain name helpers', () => {
    it('should get chain name for Aztec networks', () => {
      const responder = createTestResponder({
        matched: {
          required: {
            interfaces: [],
      ...
      const mainnetName = (adapter as any).getChainName('aztec:mainnet', ChainType.Aztec);
      const testnetName = (adapter as any).getChainName('aztec:testnet', ChainType.Aztec);
      const sandboxName = (adapter as any).getChainName('aztec:31337', ChainType.Aztec);
      const unknownName = (adapter as any).getChainName('aztec:custom', ChainType.Aztec);

      expect(mainnetName).toBe('Aztec Mainnet');
      expect(testnetName).toBe('Aztec Testnet');
      expect(sandboxName).toBe('Aztec Sandbox');
      expect(unknownName).toBe('Aztec Network');
    });

    it('should get chain name for EVM networks', () => {
      const responder = createTestResponder({
        matched: {
          required: {
            interfaces: [],
      ...
      // Access private method for testing
      const ethereumName = (adapter as any).getChainName('1', ChainType.Evm);
      const polygonName = (adapter as any).getChainName('137', ChainType.Evm);
      const arbitrumName = (adapter as any).getChainName('42161', ChainType.Evm);
      const unknownName = (adapter as any).getChainName('999999', ChainType.Evm);

      expect(ethereumName).toBe('Ethereum Mainnet');
      expect(polygonName).toBe('Polygon');
      expect(arbitrumName).toBe('Arbitrum One');
      expect(unknownName).toBe('Unknown Chain (999999)');
    });

    it('should get chain name for Solana networks', () => {
      const responder = createTestResponder({
        matched: {
          required: {
            interfaces: [],
      ...
      // Access private method for testing
      const mainnetName = (adapter as any).getChainName('solana:mainnet', ChainType.Solana);
      const devnetName = (adapter as any).getChainName('solana:devnet', ChainType.Solana);
      const testnetName = (adapter as any).getChainName('solana:testnet', ChainType.Solana);
      const unknownName = (adapter as any).getChainName('custom-cluster', ChainType.Solana);

      expect(mainnetName).toBe('Solana Mainnet');
      expect(devnetName).toBe('Solana Devnet');
      expect(testnetName).toBe('Solana Testnet');
      expect(unknownName).toBe('Solana Network');
    });
  });
