/**
 * Global error handler for production environments with SSR safety
 *
...
import { attachGlobalListener as addGlobalListener } from '../../utils/dom-essentials.js';
...
/**
 * Lazy process getter for Node.js environments
 */
...
export function initializeGlobalErrorHandler(options: GlobalErrorHandlerOptions = {}): void {
  ...
  if (isBrowser()) {
    // Browser environment
    const unhandledRejectionCleanup = addGlobalListener(
      'window',
      'unhandledrejection',
      handleUnhandledRejection as EventListener,
    );
    const errorCleanup = addGlobalListener('window', 'error', handleGlobalError as EventListener);

    state.cleanupFunctions.push(unhandledRejectionCleanup, errorCleanup);
  } else {
    // Node.js environment
    const proc = getProcess();
    if (proc) {
      const handleNodeRejection = (reason: unknown, promise: Promise<unknown>) => {
      ...
      proc.on('unhandledRejection', handleNodeRejection);
      proc.on('uncaughtException', handleNodeError);

      // Store cleanup functions
      state.cleanupFunctions.push(
        () => proc.off('unhandledRejection', handleNodeRejection),
        () => proc.off('uncaughtException', handleNodeError),
      );
    }
  }
