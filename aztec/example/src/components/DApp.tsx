 ...
 * - Connect to an Aztec wallet using WalletMesh (`AztecRouterProvider` and `connectAztec`).
 ...
 * It uses a `window.postMessage` based transport for communication with the Wallet component,
 * simulating a scenario where the DApp and Wallet might be in different frames or contexts.
 */
const DApp: React.FC = () => {
  ...
  /** Ref to a cleanup function for the message transport. */
  const cleanupRef = useRef<(() => void) | null>(null);

  /**
   * Connects the DApp to the Aztec wallet via WalletMesh.
   * Sets up a `window.postMessage` based transport for communication with the Wallet component,
   * initializes the `AztecRouterProvider`, and uses `connectAztec` to establish
   * a session and obtain an initialized `AztecDappWallet` instance.
   */
  const connectWallet = async () => {
    ...
    // Create bidirectional transport for communication with router
    const transport = {
      ...
      onMessage: (callback: (message: unknown) => void) => {
        // Set up message listener to forward responses from router
        messageHandler = (event: MessageEvent) => {
          if (event.source === window && event.data?.type === 'router_to_dapp') {
